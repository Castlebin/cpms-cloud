...
public class SqlLogInterceptor implements Interceptor {
	...
	private Method clientPreparedGetSqlMethod;
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		}else if(CLIENT_PREPARED_STATEMENT.equals(stmtClassName)) {
			try {
				if (clientPreparedGetSqlMethod == null) {
					Class<?> clazz = Class.forName(CLIENT_PREPARED_STATEMENT);
					clientPreparedGetSqlMethod = clazz.getMethod("getSql");
				}
				Object stmtSql = clientPreparedGetSqlMethod.invoke(statement);
				if (stmtSql instanceof String) {
					originalSql = (String) stmtSql;
				}
			} catch (Exception e) {
